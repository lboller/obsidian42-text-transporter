import { type Editor, Notice, type TFile, getLinkpath } from "obsidian";
import type TextTransporterPlugin from "../main";
import {
	GenericFuzzySuggester,
	type SuggesterItem,
} from "../ui/GenericFuzzySuggester2";
import { getDnpForToday, getDnpForTomorrow } from "./dailyNotesPages";
import {
	blocksWhereTagIsUsed,
	filesWhereTagIsUsed,
	getAllTagsJustTagNames,
} from "./tags";
import { ViewType, getActiveView, getActiveViewType } from "./views";
...
export async function createFileChooser(
	...
	excludeFileFromList?: string,
): Promise<GenericFuzzySuggester> {
	...
	if (excludeFileFromList)
		...
		for (let i = 0; i < fileList.length; i++) {
			if (
				fileList[i].info.localeCompare(excludeFileFromList, undefined, {
					sensitivity: "base",
				}) === 0
			) {
				fileList.splice(i, 1);
	...
	if (plugin.settings.bookmarks.trim().length > 0) {
		...
		for (let i = bookmarks.length - 1; i >= 0; i--) {
			...
			if (
				filePath === "DNPTODAY" ||
				filePath === "DNPTOMORROW" ||
				(await plugin.app.vault.adapter.exists(filePath))
			) {
				fileList.unshift({
					display: `Bookmark: ${bookmarks[i]}`,
					info: bookmarks[i],
				});
			}
		}
...
export async function convertFileIntoArray(
	plugin: TextTransporterPlugin,
	filePath: string,
): Promise<Array<SuggesterItem>> {
	const fileContentsArray: Array<SuggesterItem> = [];
	for (const [key, value] of Object.entries(
		(await plugin.app.vault.adapter.read(filePath)).split("\n"),
	))
...
export async function openFileInObsidian(
	...
	const file: TFile = plugin.app.metadataCache.getFirstLinkpathDest(
	...
	setTimeout(async () => {
		...
		editor.setSelection(
			...
			{
				line: gotoStartLineNumber + lineCount,
				ch: editor.getLine(gotoStartLineNumber + lineCount).length,
			},
		);
...
export async function parseBookmarkForItsElements(
	...
	let filePath = bookmarkCommandString.substring(
		0,
		bookmarkCommandString.search(";"),
	);
	const command = bookmarkCommandString
		.substring(filePath.length + 1)
		.toLocaleUpperCase()
		.trim();
	try {
		...
		if (await plugin.app.vault.adapter.exists(filePath)) {
			...
			if (command === "BOTTOM" || command !== "TOP") {
				if (command === "BOTTOM")
					...
					// bookmark has a location, so find in file.
					for (let i = 0; i < fileBkmrkContentsArray.length; i++) {
						if (
							fileBkmrkContentsArray[i].display.toLocaleUpperCase().trim() ===
							command
						) {
		...
		new Notice(
			`Something is wrong with the bookmark. File system reports: ${e.toString()}`,
		);
		error = 2;
	}
}
...
export async function createTagFileListChooser(
	...
	for (const tag of tagList) tagListArray.push({ display: tag, info: tag });
	...
	await tagChooser.display(async (tagChosen: SuggesterItem) => {
		...
		await tagFileChooser.display(
			async (
				fieleChosen: SuggesterItem,
				evtFile: MouseEvent | KeyboardEvent,
			) => {
				const fileContentsArray: Array<SuggesterItem> =
					await convertFileIntoArray(plugin, fieleChosen.info);
				if (showTop)
					fileContentsArray.unshift({ display: "-- Top of file --", info: -1 });
				await displayFileLineSuggesterFromFileList(
					plugin,
					returnEndPoint,
					showTop,
					fieleChosen.info,
...
export async function createTagBlockListChooser(
	...
	await tagChooser.display(async (tagChosen: SuggesterItem) => {
		...
		await tagBlockChooser.display(
			async (tagBlock: SuggesterItem, evt: MouseEvent | KeyboardEvent) => {
				callback(
					tagBlock.info.file,
					await convertFileIntoArray(plugin, tagBlock.info.file),
					tagBlock.info.position.start.line,
					tagBlock.info.position.end.line,
					evt,
				);
			},
		);
	...
	await chooser.display(
		async (
			...
			if (targetFileName === TAG_FILE_SEARCH) {
				await createTagFileListChooser(
					plugin,
					returnEndPoint,
					showTop,
					callback,
			...
			if (targetFileName === TAG_BLOCK_SEARCH) {
				await createTagBlockListChooser(
					plugin,
					returnEndPoint,
					showTop,
					callback,
			...
			if (
				targetFileName.includes("DNPTODAY") ||
				targetFileName.includes("DNPTOMORROW") ||
				targetFileName.includes(".md;")
			) {
				// a bookmark was selected with a command. process callback
				const bkmkInfo = await parseBookmarkForItsElements(
					plugin,
					targetFileName,
					pullTypeRequest,
				);
				if (shiftKeyUsed === false) {
					// bookmark location, perform the transport command
					callback(
						bkmkInfo.fileName,
						bkmkInfo.fileBookmarkContentsArray,
						bkmkInfo.fileLineNumber,
						bkmkInfo.fileLineNumber,
						evtFileSelected,
					);
					return;
				}
				// use the bookmarked location as starting point for next step in commands
			...
			const fileContentsArray: Array<SuggesterItem> =
				await convertFileIntoArray(plugin, targetFileName);
			if (showTop)
				fileContentsArray.unshift({ display: "-- Top of file --", info: -1 });
...
export async function displayFileLineSuggesterFromFileList(
	...
	targetFileName: string,
	fileContentsArray: Array<SuggesterItem>,
	fileContentsStartingLine: number,
	evtFileSelected: MouseEvent | KeyboardEvent,
	callback: FileChooserCallback,
): Promise<void> {
	...
	const lineCountArray: SuggesterItem[] = fileContentsArray.map((item) => {
		...
		return {
			display: (lineNumber > 0 ? `${lineNumber} ` : "") + item.display,
			info: item.info,
		};
	});

	if (fileContentsStartingLine > 0)
		firstLinechooser.setSuggesterData(
			lineCountArray.slice(fileContentsStartingLine),
		);
	else firstLinechooser.setSuggesterData(lineCountArray);

	await firstLinechooser.display(
		async (
			iFileLocation: SuggesterItem,
			evtFirstLine: MouseEvent | KeyboardEvent,
		) => {
			...
			if (returnEndPoint) {
				//if expecting endpoint, show suggester again
				if (startFilePosition === fileContentsArray.length - 1) {
					...
					lastLineChooser.setPlaceholder(
						"Select the last line for the selection",
					);
					await lastLineChooser.display(
						async (
							iFileLocationEndPoint: SuggesterItem,
							evetLastLine: MouseEvent | KeyboardEvent,
						) => {
							callback(
								targetFileName,
								fileContentsArray,
								startFilePosition,
								Number(iFileLocationEndPoint.info),
								evtFileSelected,
								evtFirstLine,
								evetLastLine,
							);
